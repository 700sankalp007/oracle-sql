...
SELECT contactname,max(orderid)
...
CREATE OR REPLACE FUNCTION get_laetst_order_date( p_customer_id customers.customerid%type) RETURN orders.orderdate%type
IS
...
FROM products
WHERE categoryid IN (SELECT categoryid FROM categories GROUP BY categoryid)
ORDER BY (unitsinstock);

--6. Rank customers by the total sales amount within each order date

SELECT orderdate,contactname,sum(unitprice*quantity) as total_sales
...
SELECT shippeddate,orderid ,sum(unitprice * quantity * (1 - discount)),to_char(shippeddate,'YYYY') as yyear
FROM orders INNER JOIN orderdetails
USING(orderid)
...
SELECT country,lastname,firstname,shippeddate,orderid,sum(unitprice*quantity)
FROM employees INNER JOIN orders
USING(employeeid)
INNER JOIN orderdetails
USING(orderid)
GROUP BY country,lastname,firstname,shippeddate,orderid
...
SELECT o.orderid,p.productid,p.productname,p.unitprice,o.quantity,discount,((p.unitprice*o.quantity)-(p.unitprice*o.quantity*discount))
...
CREATE VIEW OR REPLACE vw_Products_Above_Average_Price(productname,unitprice) AS 
    SELECT productname,unitprice
    FROM products
    WHERE unitprice > (SELECT avg(unitprice) FROM products);

--2. vw_Quarterly_Orders_by_Product

CREATE OR REPLACE VIEW vw_Quarterly_Orders_by_Product (productname,companyname,orderyear) AS
select productname, companyname, to_char(orderdate, 'yyyy')
...
CREATE OR REPLACE VIEW vw_Units_In_Stock AS 
SELECT country,contactname,sum(unitsinstock)
...
CREATE OR REPLACE VIEW  vw_10Most_Expensive_Products AS
    SELECT productid,productname,unitprice
    FROM products
    ORDER BY unitprice DESC
    FETCH FIRST 10 ROWS ONLY;
